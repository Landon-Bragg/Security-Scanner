...
from src.core.database import AsyncSessionLocal, Repository, SecurityFinding, ScanJob, FindingType, SeverityLevel
...
class ScannerWorker:
    ...
    async def process_push_event(self, event_data: Dict[str, Any]):
        ...
        async with AsyncSessionLocal() as db:
            repo_record = await self._get_or_create_repository(db, payload.get('repository', {}))
    ...
    async def _get_or_create_repository(self, db, repo_data: Dict[str, Any]) -> Repository:
        """Get or create repository record"""
